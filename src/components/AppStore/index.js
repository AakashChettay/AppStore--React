...
class AppStore extends Component {
  ...
  componentDidMount() {
    this.filterAppsList('SOCIAL')
  }

  pushRelevantProjects = category => {
    this.filterAppsList(category)
  }
  ...
  searchBasedOnInput = event => {
    ...
    this.setState({searchText}, () => {
      this.filterAppsList(projectCategory)
    })
  }

  filterAppsList = category => {
    ...
    this.setState({projectCategory: category, displayProjectList: filteredList})
  }

  render() {
    const {projectCategory, displayProjectList} = this.state
    return (
        ...
        <ul className="tabs-container">
          {tabsList.map(tabDetails => (
            <TabItem
              ...
              pushRelevantProjects={this.pushRelevantProjects}
              isActiveTab={projectCategory === tabDetails.tabId}
            />
          ))}
        </ul>

        <ul className="project-list-container">
          {displayProjectList.map(projectDetails => (
            <AppItem
